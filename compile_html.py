  ...
  <style>
    ...
    * {
    ...
    .mole-grid {
      ...
      aspect-ratio: 1;
      padding: 24px;
      background: radial-gradient(circle, #8EDAD0, #56AEA0); /* Premium Teal Grass Gradient */
      border: 4px solid var(--border-color);
      border-radius: 32px;
      box-shadow: inset 4px 4px 12px rgba(0,0,0,0.15), 6px 6px 0px var(--border-color);
    }
    
    .mole-hole {
      position: relative;
      background: radial-gradient(circle at center, #2C1A1A 60%, #150F0E 100%); /* Deep dark ground depth */
      border: 4px solid var(--border-color);
      border-radius: 50% / 35%; /* Smooth rounded oval shape */
      overflow: visible; /* Sign can pop out because the character itself is masked */
    ...
    .mole-mask {
      ...
      width: 100%;
      height: 100%;
      overflow: hidden; /* This masks the mole when it retreats! */
      border-radius: 50% / 35%;
      z-index: 2;
      pointer-events: none; /* Let clicks pass to the mole inside */
    }
    
    .mole-character {
      width: 82px;
      height: 95px;
      background: linear-gradient(135deg, #DE9C68, #A96131);
      border: 4px solid var(--border-color);
      border-radius: 42px 42px 0 0;
      position: absolute;
      bottom: -110px; /* Hidden below the hole bottom */
      left: 50%;
      transform: translateX(-50%);
      transition: bottom 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.25);
      cursor: pointer;
      display: flex;
      flex-direction: column;
      align-items: center;
    ...
    /* Cute Ears for the Mole */
    .mole-character::before {
      content: "";
      position: absolute;
      left: 10px;
      top: -8px;
    ...
    .mole-character.up {
      bottom: 15px; /* Fully visible, standing tall */
    }
    ...
    .mole-nose {
      width: 14px;
      height: 8px;
      background: #FF8A8A; /* Pink cute nose */
      border: 2.5px solid var(--border-color);
      border-radius: 50%;
      margin-top: 2px;
      z-index: 5;
    ...
    .mole-dirt-cover {
      width: 108%;
      height: 32px;
      background: linear-gradient(180deg, #A27653, #785233); /* Beautiful dirt gradient */
    ...
    .mole-sign {
      position: absolute;
      top: -95px; /* Sits higher to accommodate larger font */
      left: 50%;
      background: #FFFFFF;
      border: 4px solid var(--border-color);
      border-radius: 16px;
      padding: 10px 14px;
      font-size: 18px; /* Enlarged font size for maximum readability */
      font-weight: 900;
      width: 200px; /* Increased width to prevent clipping */
      text-align: center;
      box-shadow: 4px 4px 0px var(--border-color);
      pointer-events: none;
      ...
      z-index: 10;
      opacity: 0;
      transform: translateX(-50%) scale(0.7) translateY(15px);
      transform-origin: bottom center;
...
<body>
  ...
  <div id="app-container">
    ...
    <div class="view active" id="view-home">
      <div class="home-header" style="display: flex; justify-content: space-between; align-items: center; gap: 20px; border-bottom: 3px solid var(--border-color); padding-bottom: 12px; margin-bottom: 16px;">
        <div>
          <h1 class="home-title" style="margin: 0; font-size: 28px;">🛡️ 小小資安與健康生活守護者</h1>
          <p class="home-subtitle" style="margin: 4px 0 0 0; font-size: 14px;">跟著粉紅果凍人一起挑戰關卡，學習如何保護自己的安全與建立健康習慣吧！</p>
        </div>
        <button class="btn-resetProgress" style="margin: 0; background: var(--primary-blue); white-space: nowrap; box-shadow: 3px 3px 0px var(--border-color);" onclick="openTeacherSettings()">
          ⚙️ 教師後台設定
        </button>
      </div>
  ...
  <script>
    ...
    const moleStatements = [
      { text: "不透露身分證字號與地址", isCorrect: true },
      { text: "密碼複雜且不隨意分享", isCorrect: true },
      { text: "收到可疑簡訊先撥打165", isCorrect: true },
      { text: "用溫暖同理心與網友聊天", isCorrect: true },
      { text: "控制玩手機時間適度休息", isCorrect: true },
      { text: "網友約見面先告知父母", isCorrect: true },
      { text: "只從官方商店安全下載軟體", isCorrect: true },
      { text: "尊重他人隱私不散布照片", isCorrect: true },
      { text: "不隨便點擊未知連結信件", isCorrect: true },
      { text: "懷疑被詐騙立刻尋求大人協助", isCorrect: true },
    ...
    function submitGame3Timeline() {
      ...
      // Regular meals: count of 'eat' slots
      const eatHours = state.game3.timeline.filter(x => x && x.id === 'eat').length;
      let eatState = "good";
      if (eatHours <= 2) eatState = "hungry";
      else if (eatHours >= 4) eatState = "full";
      ...
      const indicators = [
        ...
        {
          title: "規律飲食",
          status: eatState === "hungry" ? "沒吃飽 ❌" : (eatState === "full" ? "吃太撐 ❌" : "定時定量 👍"),
          color: eatState === "hungry" || eatState === "full" ? "var(--primary-red-dark)" : "var(--primary-green-dark)",
          text: eatState === "hungry" ? "肚子咕嚕嚕叫，今天好像吃太少了⋯" : (eatState === "full" ? "肚子好撐喔，吃太多餐了啦⋯" : "一日三餐定時定量，營養又均衡！"),
          icon: eatState === "hungry" || eatState === "full" ? "🤢" : "😋",
          jellySpeech: eatState === "hungry" ? "肚子咕嚕嚕叫，今天好像吃太少了⋯" : (eatState === "full" ? "肚子好撐喔，吃太多餐了啦⋯" : "一日三餐剛剛好，感覺元氣滿滿！"),
          jellyFilter: eatState === "hungry" ? "grayscale(0.3) saturate(0.8)" : (eatState === "full" ? "hue-rotate(280deg) saturate(1.5)" : "none")
        },
        {
          title: "功課與學習",
      ...
      indicators.forEach((ind, idx) => {
        setTimeout(() => {
          ...
          if (idx === indicators.length - 1) {
            setTimeout(() => {
              // Final summary word calculation based on screenHours
              let summaryText = "";
              let finalFilter = "none";
              
              if (screenHours <= 1) {
                summaryText = "設計得很均衡！果凍人很感謝你 🌟";
                finalFilter = "none";
                createConfetti();
              } else if (screenHours === 2) {
                summaryText = "不錯，再調整一下會更好 👍";
                finalFilter = "none";
              } else {
                summaryText = "果凍人有點辛苦，試試看少一點螢幕時間 😅";
                finalFilter = "grayscale(0.3) saturate(0.8)";
              }
    ...
    function hideMole(index) {
      ...
      if (mole) {
        mole.classList.remove('up');
        mole.classList.remove('whacked');
        mole.style.background = ''; // Reset gradient
      }
      const sign = document.getElementById('mole-sign-' + index);
      if (sign) {
        sign.classList.remove('up');
        sign.style.background = ''; // Reset background
        sign.style.borderColor = ''; // Reset border color
        sign.style.color = ''; // Reset text color
      }
    ...
    function spawnMole() {
      ...
      // Select statement: 70% good (isCorrect: true), 30% bad (isCorrect: false)
      const goodStatements = moleStatements.filter(s => s.isCorrect);
      const badStatements = moleStatements.filter(s => !s.isCorrect);
      const isGood = Math.random() < 0.7;
      const statement = isGood 
        ? goodStatements[Math.floor(Math.random() * goodStatements.length)]
        : badStatements[Math.floor(Math.random() * badStatements.length)];
      ...
      mole.style.background = 'linear-gradient(135deg, #DE9C68, #A96131)';
    ...
    function whackMole(index) {
      ...
      if (isCorrect) {
        game4Score++;
        mole.style.background = 'linear-gradient(135deg, #A2F39C, #2ECC71)'; // Bright green gradient!
        if (sign) {
          sign.style.background = '#E8F8F5';
          sign.style.borderColor = '#2ECC71';
          sign.style.color = '#27AE60';
        }
        playBeep(600, 150);
        showFloatingText(mole, "+1");
      } else {
        game4Score--;
        mole.style.background = 'linear-gradient(135deg, #FF7675, #D63031)'; // Bright red gradient!
        if (sign) {
          sign.style.background = '#FDEDEC';
          sign.style.borderColor = '#E74C3C';
          sign.style.color = '#C0392B';
        }
    ...
    function showFloatingText(element, text) {
      ...
      floatText.style.fontSize = '36px'; // Substantially larger text
      floatText.style.fontWeight = '900';
      floatText.style.color = text.startsWith('+') ? '#2ECC71' : '#E74C3C';
      floatText.style.textShadow = '3px 3px 0px #000'; // High contrast black outline shadow
      floatText.style.pointerEvents = 'none';
      floatText.style.zIndex = '999';
      floatText.style.transition = 'all 0.8s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
      floatText.style.transform = 'translate(-50%, 0) scale(0.5)';
      
      container.appendChild(floatText);

      setTimeout(() => {
        floatText.style.transform = 'translate(-50%, -70px) scale(1.3)';
